...
puts array.size
# --> fonction native de ruby, fais une boucle et compte automatiquement.
...
puts i
# création d'une boucle et compte les éléments compris entre 0 et 9 pour sortir les adresses avec numéros.



def include_aude(array)
   array.count {|item| item.match("[Aa]ude")}
 end
 puts include_aude (array)
# uttilisation de la fonction native .count avec [Aa] pour définir les items avec majuscule et sans majuscule.



def starwith_maj(array)
   array.count {|item| item.match(/^[A-Z]/)}
 end
 puts starwith_maj(array)


def include_maj(array)
  array.count {|item| item.match(/[A-Z]/)}
end
puts include_maj(array)

 # création d'une boucle avec comptage des elements en majuscule en excluant @.
